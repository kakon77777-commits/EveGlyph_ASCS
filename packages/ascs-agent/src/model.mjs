...
export function validateContextPack(value) {
  ...
  if (plain(value.tool_manifest)) { required(errors,value.tool_manifest,['manifest_id','tools'],'tool_manifest'); extras(errors,value.tool_manifest,new Set(['manifest_id','tools']),'tool_manifest'); str(errors,value.tool_manifest.manifest_id,'tool_manifest.manifest_id',{min:1}); if (array(errors,value.tool_manifest.tools,'tool_manifest.tools')) for (let i=0;i<value.tool_manifest.tools.length;i++) { const t=value.tool_manifest.tools[i]; const p=`tool_manifest.tools[${i}]`;  if (!plain(t)) { object(errors,t,p); continue } required(errors,t,['name','effect_class','schema_hash'],p); extras(errors,t,new Set(['name','effect_class','schema_hash']),p); str(errors,t.name,`${p}.name`,{min:1}); enm(errors,t.effect_class,`${p}.effect_class`,TOOL_EFFECTS); str(errors,t.schema_hash,`${p}.schema_hash`,{pattern:SHA256}) } } else object(errors,value.tool_manifest,'tool_manifest')
...
export function validateAgentRun(value) {
  ...
  for (const [name, pattern] of [['proposal_ids',UUID_URNP],['external_effect_ids',UUID_URN]]) uniqueStrings(errors,value[name],name,pattern)
